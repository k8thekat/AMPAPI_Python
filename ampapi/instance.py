...
class AMPInstance(
    ...
    def __init__(
        ...
        # LOGGER.debug("DEBUG %s __init__ %s", type(self).__name__, id(self))
        ...
        self.url = f"{self._bridge.url}/API/ADSModule/Servers/{self._instance_id}"

    def __hash__(self) -> int:
        return hash(self._instance_id)

    def __eq__(self, other: object) -> bool:
        return isinstance(other, self.__class__) and self._instance_id == other._instance_id
    ...
    async def get_instance_status(self) -> Union[AMPInstance, AMPMinecraftInstance] | ActionResultError:
        ...
        """
        result: Instance | ActionResultError = await self._controller.get_instance(instance_id=self._instance_id)  # type: ignore -- See the @has_controller decorator.
        if isinstance(result, ActionResultError):
            LOGGER.warning(
